...
-- Refresh: incremental (subscription changes)
-- Coverage: ~37% of registered users
...
CREATE TABLE {schema}.pushly_subscribers (
    ...
    last_push_sent_at   TIMESTAMP   NULL,
    push_open_count     INTEGER     NOT NULL DEFAULT 0,
    created_at          TIMESTAMP   NOT NULL DEFAULT NOW(),
    updated_at          TIMESTAMP   NOT NULL DEFAULT NOW()
);

ALTER TABLE {schema}.pushly_subscribers
    ADD CONSTRAINT uq_pushly_external_id UNIQUE (external_id);

ALTER TABLE {schema}.pushly_subscribers
    ADD CONSTRAINT fk_pushly_subscribers_user_id
    FOREIGN KEY (user_id) REFERENCES {schema}.zephr_users(user_id)
    ON DELETE CASCADE;

ALTER TABLE {schema}.pushly_subscribers
    ADD CONSTRAINT chk_pushly_platform
