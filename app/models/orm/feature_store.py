...
class FeatureStore(Base):
    """Computed user-level feature matrix — one row per registered user, 64 columns.

    Written by the pipeline feature builder (Step 3) and ML write-back (Step 8).
    Upserted weekly via INSERT ... ON CONFLICT (user_id) DO UPDATE.
    """
    ...
    # --- Email metadata flags — NOT in ML matrix (4) ---
